...
CRON_CMD="$CRON_SCHEDULE bash $REPO_DIR/utils/update-research-cron.sh >> $LOG_DIR/cron.log 2>&1"
...
echo "  Logs:      $LOG_DIR/run_*/ (per-job directories, 7-day retention)"
echo "  Latest:    $LOG_DIR/latest/ (symlink to most recent run)"
echo "  Cron log:  $LOG_DIR/cron.log"
echo ""
echo "Useful commands:"
echo "  View latest log:    cat $LOG_DIR/latest/update.log"
echo "  View screenshots:   ls $LOG_DIR/latest/*.png"
echo "  Force run:          bash $REPO_DIR/utils/update-research-cron.sh --force"
